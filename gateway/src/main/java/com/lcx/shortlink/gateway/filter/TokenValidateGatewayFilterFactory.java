...
package com.lcx.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.lcx.shortlink.gateway.config.Config;
import com.lcx.shortlink.gateway.dto.GatewayErrorResult;
import com.lcx.shortlink.gateway.toolkit.HashUtil;
import com.lcx.shortlink.gateway.toolkit.JwtTools;
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                try {
                    if (StringUtils.hasText(username) && StringUtils.hasText(token)) {
                        String hash1 = HashUtil.hash(username);
                        String hash2 = (String) JwtTools.parseToken(token).get("username");

                        if (hash2.equals(hash1)) {
                            String userInfo = Optional.ofNullable(stringRedisTemplate.opsForValue().get("short-link:login:" + username))
                                    .orElseThrow(() -> new RuntimeException("用户身份校验异常"));
                            ...
                            throw new RuntimeException("用户身份校验异常");
                        }
                    } else {
